...
## 二·补 轮播图必须真正可用（高频翻车点，务必照做）
如果页面里出现"轮播图 / carousel / 走马灯 / 图片切换"，必须用下面这套**绝不出错**的写法，禁止凭空发挥：
1. 结构：一个相对定位容器 .carousel，里面 N 张 .slide 绝对定位叠在一起（position:absolute; inset:0），用 opacity + transition 做淡入淡出切换——**不要用 translateX/百分比位移**（位移算错就会出现空白或半张图，这是最常见的 bug）。
2. 只显示当前张：写一个 show(i) 函数，遍历所有 slide，给当前张 opacity:1、其余 opacity:0；同步高亮第 i 个圆点。所有索引都用 \`(i + N) % N\` 取模，保证首尾循环、不越界。
3. 必备控件：左右各一个「‹ / ›」箭头按钮（prev/next），底部一排可点击的圆点（dots）。箭头和圆点都是 <button>，尺寸 ≥ 44×44px，带 aria-label。
4. 自动播放：用 setInterval 每 4–5 秒 next() 一次；鼠标移入容器时 clearInterval 暂停、移出恢复；尊重 prefers-reduced-motion（reduce 时不自动播放）。
5. 容器必须有确定高度（如固定 aspect-ratio 或 min-height），否则绝对定位的 slide 会让容器塌成 0 高、看起来"轮播图不见了"。
6. 键盘可用：← / → 切换。
7. 自检：渲染后第一张必须可见、点箭头/圆点能切换、自动轮播在动——三者缺一不可。

## 二·再补 文字轮播 / 评价滚动同理
评价、引语等"文字轮播"用同一套 show(i) + 圆点 + 自动播放逻辑，同样要保证容器有 min-height，避免切换时高度跳动。
...
## 五、内容与结构（页面要"够长够饱满"，不要交一屏就完事）
- Hero 首屏占满视口高度（min-h-screen / 100dvh），一进来就有分量。
- Hero 之后至少再有 **5 个**有实质内容的区块，按主题从下面取舍：特性/亮点、流程/时间线、数据/图表、内容展示/作品、用户评价、价格/方案、常见问答 FAQ、行动号召 CTA、页脚。**整页正常滚动应远超一屏**（桌面端至少 3–4 屏高）。
- 每个区块上下留足呼吸：纵向内边距 py-20 / py-24（移动端可降到 py-12），区块之间层次分明，不要把所有内容挤在一屏里。
- 每个区块都要有真内容：标题 + 小标题 + 2~4 段/卡片的具体文案，不能只有一个孤零零的标题。
- 文案具体、可信、贴合用户主题，绝不用「Lorem ipsum」之类占位文字。
- 响应式：移动端到桌面端都要好看；移动端不出现横向滚动条。
- 配色克制、版式有节奏：统一的最大宽度容器（如 max-w-6xl mx-auto px-6）、一致的标题字阶，让排版看起来是"设计过的"。
